...
- **Optional slots stay optional.** A variant that supplies no `renderNotice`, no
  ...
  holds for `decisionTab`: avvikelse fills it with the lex Sarah decision (the Beslut errand tab,
  offered only on reported-misconduct errands with a decision document the user reaches); a variant
  without the slot gets no tab.
