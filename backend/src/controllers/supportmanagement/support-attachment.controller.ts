...
export class SupportAttachmentController {
  ...
  @Get('/supportattachments/errands/:id/attachments/:attachmentId')
  ...
  async fetchSingleSupportAttachment(
    ...
    const url = `${this.SERVICE}/${MUNICIPALITY_ID}/${this.namespace}/errands/${id}/attachments/${attachmentId}`;
  ...
  @Get('/supportattachments/errands/:id/attachments')
  @OpenAPI({ summary: 'Get attachments for errand' })
  @UseBefore(authMiddleware)
  async fetchSupportAttachments(@Req() req: RequestWithUser, @Param('id') id: string, @Res() response: any): Promise<SupportAttachment[]> {
    const url = `${this.SERVICE}/${MUNICIPALITY_ID}/${this.namespace}/errands/${id}/attachments`;
    const res = await this.apiService.get<SupportAttachment[]>({ url }, req.user);
    return response.status(200).send(res.data);
  }

  @Delete('/supportattachments/errands/:id/attachments/:attachmentId')
  ...
  @Post('/supportattachments/errands/:id/attachments')
  @HttpCode(201)
