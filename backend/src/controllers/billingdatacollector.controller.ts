...
import { ScbKpiResponse, ScheduledBilling } from '@/data-contracts/billing-data-collector/data-contracts';
...
export class BillingDataCollectorController {
  ...
  @Get('/billingdatacollector/kpi')
  @OpenAPI({ summary: 'Get KPI index from SCB' })
  @UseBefore(authMiddleware)
  async getKpi(
    @Req() req: RequestWithUser,
    @QueryParam('baseYear') baseYear: string,
    @QueryParam('period') period: string,
    @Res() response: Response<ScbKpiResponse>,
  ): Promise<Response<ScbKpiResponse>> {
    const url = `${this.SERVICE}/${MUNICIPALITY_ID}/scb/kpi?baseYear=${baseYear}&period=${period}`;
    const res = await this.apiService.get<ScbKpiResponse>({ url }, req.user);
