  ...
  describe('Errand page', () => {
    ...
    it('allows updating errand information', () => {
      ...
      cy.wait('@patchErrand').should(({ request, response }) => {
        ...
        expect(request.body.labels.map((label) => label.name)).to.include('ELECTRICITY_SERVANET');
        expect(request.body.labels.map((label) => label.name)).to.include('ELECTRICITY_SERVANET.EMPLOYMENT');
        expect(request.body.labels.map((label) => label.name)).to.include(
