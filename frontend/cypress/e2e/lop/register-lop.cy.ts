...
onlyOn(Cypress.env('application_name') === 'LOP', () => {
  describe('register page', () => {
    ...
    it('sends the correct data for new errand', () => {
      ...
      delete labelCat?.labels; // Remove subtype to simplify test
      ...
      cy.wait(`@updateErrand`).should(({ request, response }) => {
        ...
        expect(request.body.labels.map((label) => label.name)).to.include(labelCat!.name!);
        expect(request.body.labels.map((label) => label.name)).to.include(labelType!.name!);
