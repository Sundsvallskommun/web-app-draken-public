...
/**
 * The minimum SupportManagement API needed to boot and open an errand. It is shared by
 * AOT's detailed integration suite and the per-dragon deployment smoke tests. Empty runtime
 * flags preserve the selected dragon's real environment configuration; the application and
 * errand number follow the build identity. Investigation-only requests are recorded so a
 * plain SupportManagement deployment cannot silently start using Avvikelse.
 */
...
export const supportLabelFixture = { category, type, subtype } as const;
...
/**
 * What the BFF actually returns for AOT: `getSupportApplicationProfile` finds no registered
 ...
 */
const emptySupportApplicationProfile = () => ({
  application,
...
export interface SupportAppTrace {
...
export interface SupportAppScenario {
...
export async function installSupportAppMock(page: Page, scenario: SupportAppScenario = {}): Promise<SupportAppTrace> {
  const trace: SupportAppTrace = {
    profileGets: 0,
  ...
  await page.route(`${backendOrigin}/**`, async (route: Route) => {
    ...
    if (method === 'GET' && path.endsWith('/supportmanagement/application-profile')) {
      ...
      await fulfillJson(
        route,
        scenario.supportProfile === undefined ? emptySupportApplicationProfile() : scenario.supportProfile,
        scenario.supportProfileStatus ?? 200
      );
      return;
    }
