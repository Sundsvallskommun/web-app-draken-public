...
// @sk-web-gui bump:
//  - `contact-externalIdType-*` / `contact-role-*` are now hidden inputs, so we
//    assert their value instead of visibility.
//  - PATCH/save requests fire synchronously on click, so request listeners are
//    registered (Promise.all) before the triggering click to avoid a race.
...
export const supportManagementPersonSearch = async (page: Page) => {
  ...
  // Validation now runs on search; click Sök to surface the error message.
  ...
  await expect(page.locator('[data-cy="contact-form"] button').filter({ hasText: 'Sök' })).toBeEnabled();
  // Re-search to re-run validation; a valid number clears the error message.
  await page.locator('[data-cy="contact-form"] button').filter({ hasText: 'Sök' }).click();
...
export const supportManagementEmployeeSearch = async (page: Page) => {
  ...
  await expect(
    page.locator('[data-cy="contact-personNumber-owner"]').locator('..').locator('button').first()
  ).toBeEnabled();
  await expect(page.locator('[data-cy="personal-number-error-message"]')).not.toBeVisible();
};
...
export const displayManuallyAddStakeholderModal = async (page: Page) => {
  ...
  // externalIdType / role are hidden inputs; assert their value, not visibility.
...
export const clearSearchResultOnPersonNumberChange = async (page: Page, mockAdressResponse: any) => {
  ...
  // Change personnumber to an invalid value and re-search to surface validation.
  await page.locator('[data-cy="contact-personNumber-owner"]').type('1');
  await expect(page.locator('[data-cy="contact-form"] button').filter({ hasText: 'Sök' })).toBeEnabled();
  await page.locator('[data-cy="contact-form"] button').filter({ hasText: 'Sök' }).click();
  await expect(page.locator('[data-cy="personal-number-error-message"]')).toBeVisible();

  // Open manual form, it should be empty. The manual form no longer renders a
  // dedicated personNumber field, so assert the name/address fields are empty.
  await page.locator('[data-cy="add-manually-button-owner"]').click();
...
export const searchAndSaveContactPersonStakeholder = async (
  ...
  await expect(page.locator('[data-cy="contact-form"] button').filter({ hasText: 'Sök' })).toBeEnabled();
  await expect(page.locator('[data-cy="personal-number-error-message"]')).not.toBeVisible();
  // Register the address lookup listener before triggering the search to avoid a race.
  await Promise.all([
    page.waitForResponse((resp) => resp.url().includes('address')),
  ...
  // Add the contact to the form; the stakeholder card shows the staged contact
  // (a refetch after save may clear it, so assert it here).
  await page
