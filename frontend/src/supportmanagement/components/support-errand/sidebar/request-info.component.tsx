...
export const RequestInfoComponent: React.FC<{ disabled: boolean }> = ({ disabled }) => {
  ...
  const handleRequestInfo = (data: RequestInfoFormProps) => {
    setIsLoading(true);
    setError(false);
    return requestInfo(user, supportErrand, municipalityId, data, addedAttachment)
      .then(() => {
        toastMessage({
          position: 'bottom',
          closeable: false,
          ...
          <Modal show={showModal} label="Begär komplettering" className="w-[52rem]" onClose={() => setShowModal(false)}>
            ...
            <Modal.Footer>
              <Button
                ...
                className="w-full"
                loading={isLoading}
                loadingText="Begär komplettering"
                onClick={() => {
                  confirm
                    .showConfirmation(
                      'Begär komplettering',
                      'Vill du begära kompletteringen?',
                      'Ja',
                      'Nej',
                      'info',
                      'info'
