...
export const buildDefaultEscalationEmailContent = (e: SupportErrand, user: string): string => {
  ...
  <br><p><b>Inkom via:</b> ${channel}</p>
  ${e.channel === 'EMAIL' ? `<p><b>Ämnesrad:</b> ${e.title}</p>` : ''}
  <p><b>Ärendet registrerades:</b> ${dayjs(e.created).format('YYYY-MM-DD HH:mm')}</p>
  <p><b>Ärendenummer i Draken:</b> ${e.errandNumber}</p>
  <br>${description ? `<b>Ärendebeskrivning</b><p>${description}</p>` : ''}
  <br>
  ${propertyDesignations?.length ? `<p><b>Fastighetsbeteckningar:</b> ${propertyDesignations.join(', ')}</p>` : ''}
  ${
    cus
      ? `
    <p><b>Kontaktuppgifter</b></p><br>
    <p><b>Namn:</b> ${extractContactInfo(cus).name}<br><b>Adress:</b> ${
          extractContactInfo(cus).adress
        }<br><b>Telefonnummer:</b> ${extractContactInfo(cus).phone}<br><b>E-postadress:</b> ${
          extractContactInfo(cus).email
        }</p>`
      : ''
  }
  ${
    contacts.length
      ? `
    <br><p><b>Övriga kontaktuppgifter</b></p>

    ${contacts
      .map((c) => {
        const info = extractContactInfo(c);
        return `<br><p><b>Namn:</b> ${info.name}<br><b>Adress:</b> ${info.adress}<br><b>Telefonnummer:</b> ${info.phone}<br><b>E-postadress:</b> ${info.email}</p>`;
      })
      .join('')}
  `
      : ''
  }
  <br><p>Har detta meddelande inte hamnat rätt?</p>
  <br><p>Hjälp gärna till att så snabbt som möjligt guida meddelandet till rätt verksamhet eller person.</p>
  <br><p>Vi önskar en fortsatt fin dag.</p>
