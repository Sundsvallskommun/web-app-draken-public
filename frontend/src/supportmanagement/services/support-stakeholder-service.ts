...
export const updateSupportErrandStakeholders: (
  errandId: string,
  municipalityId: string,
  formdata: Partial<RegisterSupportErrandFormModel>
) => Promise<boolean> = async (errandId, municipalityId, formdata) => {
...
) => Promise<boolean> = async (stakeholder, municipalityId, errand) => {
  if (errand.customer.length > 0) {
    Promise.reject('Owner already exists');
  }
  const customer: SupportStakeholderFormModel[] = [stakeholder];
  errand.contacts.forEach((c, idx) => {
    if (c.internalId === stakeholder.internalId) {
      errand.contacts.splice(idx, 1);
    }
  });
  const stakeholders = buildStakeholdersList({ customer, contacts: errand.contacts });
  const data: Partial<SupportErrandDto> = {
    stakeholders,
  };

  return apiService
    .patch<ApiSupportErrand, Partial<SupportErrandDto>>(`supporterrands/${municipalityId}/${errand.id}`, data)
    .then((res) => {
      return true;
    })
    .catch((e) => {
      console.error('Something went wrong when making owner');
      return false;
    });
};
