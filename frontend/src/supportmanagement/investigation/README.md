...
Implementationerna ligger i `src/avvikelse/` och `src/dragons/aot/investigation/` (den senare är fortfarande en platshållare).

Poängen med uppdelningen: **delad kod frågar aldrig vilken app som kör**. Den frågar vilken
funktionalitet som är påslagen. En drake väljer implementation i sin `dragons/<id>/application.ts` och aktiverar den med en capability;
en ny utredning är en ny modul som kopplas in av den berörda drakens applikation.
...
| `investigation-variant-registry.ts`          | Tar emot drakens implementationer och väljer aktiv variant. Importerar ingen implementation.                                                                                                                |
...
- **Delad kod importerar aldrig ur en variantkatalog.** Kompositionen sker i `dragons/<id>/application.ts`.
