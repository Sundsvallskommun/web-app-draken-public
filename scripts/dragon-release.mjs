...
try {
  if (!['validate', 'compose', 'verify-images'].includes(command) || !file) {
    throw new Error('Usage: yarn dragon:release <validate|verify-images> <release.json> | yarn dragon:release compose <release.json> <secret-directory>');
  }
  ...
  if (command === 'compose') {
    ...
    if (command === 'verify-images') {
      for (const side of ['frontend', 'backend']) {
        const [image] = JSON.parse(execFileSync('docker', ['image', 'inspect', release[side].image], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }));
        const labels = image?.Config?.Labels;
        if (labels?.['se.sundsvall.draken.id'] !== release.dragon || labels?.['se.sundsvall.draken.service'] !== side || labels?.['org.opencontainers.image.revision'] !== release.revision) {
  ...
  process.stderr.write((error instanceof Error && !('stderr' in error) ? error.message : 'Image verification failed; pull the digest-pinned images and check their build metadata.') + '\n');
  process.exitCode = 1;
}
