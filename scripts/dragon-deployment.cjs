...
const SIDES = ['frontend', 'backend'];
const SECRET_NAMES = new Set(['CLIENT_KEY', 'CLIENT_SECRET', 'SECRET_KEY', 'REDIS_PASSWORD', 'SAML_PRIVATE_KEY', 'SAML_PUBLIC_KEY', 'SAML_IDP_PUBLIC_CERT', 'HEALTH_USERNAME', 'HEALTH_PASSWORD']);
...
const APPLICATION_NAMES = /^(?:NEXT_PUBLIC_|SAML_|SUPPORTMANAGEMENT_|SUPPORT_INVESTIGATION_|CASEDATA_|AUTHORIZED_GROUPS$|ADMIN_GROUP$|SUPERADMIN_GROUP$|DEVELOPER_GROUP$|API_BASE_URL$|ADMINPANEL_URL$|MUNICIPALITY_ID$|DOMAIN$|DOMAIN_NAME$|BASE_PATH$|ADMIN_URL$|ORIGIN$|BASE_URL_PREFIX$|CLIENT_KEY$|CLIENT_SECRET$|SECRET_KEY$|HEALTH_|SESSION_|REDIS_|ENVIRONMENT$|CREDENTIALS$|SWAGGER_ENABLED$|LOG_DIR$|LOG_RETENTION_DAYS$)/;

function fail(message) { throw new Error(`Invalid dragon deployment: ${message}`); }
...
function assertSafeRuntimeEnvironment(environment) {
  if (['NEXT_PUBLIC_USE_AVVIKELSE_INVESTIGATION', 'NEXT_PUBLIC_USE_AOT_INVESTIGATION'].some(name => environment[name] !== undefined && environment[name] !== '')) {
...
function url(value, label, production) {
  let parsed;
  try { parsed = new URL(value); } catch { fail(`${label} must be an absolute URL`); }
...
function validateRelease(value) {
  ...
  for (const side of SIDES) {
    ...
    if (!/^[a-zA-Z0-9][a-zA-Z0-9./:_-]*@sha256:[a-f0-9]{64}$/u.test(string(service.image, `${side}.image`))) fail(`${side}.image must be pinned by digest`);
    ...
    for (const [name, value] of Object.entries(service.environment)) {
      if (!/^[A-Z][A-Z0-9_]*$/u.test(name) || RESERVED_NAMES.test(name) || SECRET_NAMES.has(name)) fail(`${side}.environment contains a reserved or secret key`);
      if (['NEXT_PUBLIC_USE_AVVIKELSE_INVESTIGATION', 'NEXT_PUBLIC_USE_AOT_INVESTIGATION'].includes(name)) fail('Retired investigation flags must be migrated before deployment');
    ...
    for (const [name, reference] of Object.entries(service.secrets)) {
  ...
  if (Object.hasOwn(backend, 'NEXT_PUBLIC_USE_INVESTIGATION')) fail('Investigation activation is declared once in frontend.environment and shared with backend');
  if (frontend.NEXT_PUBLIC_USE_INVESTIGATION !== undefined && !['true', 'false'].includes(frontend.NEXT_PUBLIC_USE_INVESTIGATION)) fail('NEXT_PUBLIC_USE_INVESTIGATION must be true or false');
  if (Object.hasOwn(frontend, 'LOG_RETENTION_DAYS')) fail('LOG_RETENTION_DAYS belongs to backend log files');
  if (Object.hasOwn(backend, 'LOG_RETENTION_DAYS') && (!/^[1-9][0-9]*$/u.test(backend.LOG_RETENTION_DAYS) || !Number.isSafeInteger(Number(backend.LOG_RETENTION_DAYS)))) {
  ...
  for (const name of ['API_BASE_URL', 'MUNICIPALITY_ID', 'AUTHORIZED_GROUPS', 'ADMIN_GROUP', 'DEVELOPER_GROUP', 'DOMAIN', 'ORIGIN', 'BASE_URL_PREFIX']) string(backend[name], `backend.${name}`);
  for (const name of REQUIRED_SECRETS) if (!release.backend.secrets[name]) fail(`backend.secrets.${name} is required`);
  if (!/^[0-9]{4}$/u.test(backend.MUNICIPALITY_ID) || frontend.NEXT_PUBLIC_MUNICIPALITY_ID !== backend.MUNICIPALITY_ID) fail('municipalities must match');
  ...
  if (domain === 'supportmanagement') {
    if (!['stable', 'sprint', 'alktsprint'].includes(backend.SUPPORTMANAGEMENT_API_TARGET)) fail('explicit SupportManagement API target is required');
  }
  ...
  if (publicApi.pathname.replace(/\/$/u, '') !== backend.BASE_URL_PREFIX.replace(/\/$/u, '')) fail('frontend API prefix must match the backend route prefix');
  ...
  if (production && (backend.ENVIRONMENT === 'LOCAL' || backend.SESSION_MEMORY === 'true')) fail('production cannot use local cookies or memory sessions');
  return release;
}

function readRelease(file) {
  let value;
  try { value = JSON.parse(readFileSync(file, 'utf8')); } catch { fail('release file is missing or invalid JSON'); }
  return validateRelease(value);
}

function readBuild(file) {
  let build;
  try { build = JSON.parse(readFileSync(file, 'utf8')); } catch { fail('immutable build metadata is missing or invalid'); }
  record(build, 'build');
  if (!Object.hasOwn(dragons, build.id) || typeof build.revision !== 'string' || !/^[a-f0-9]{40}$/u.test(build.revision)) fail('invalid immutable build identity');
  return { id: build.id, revision: build.revision };
}

function deploymentIdentity(release) {
  validateRelease(release);
  return { dragon: release.dragon, revision: release.revision, deployment: createHash('sha256').update(JSON.stringify(sorted(release))).digest('hex') };
}

function runtimeEnvironment(side, build, release, inherited, secretDirectory) {
  if (!SIDES.includes(side)) fail('unknown service');
  ...
  for (const [name, reference] of Object.entries(service.secrets)) {
    let value;
    try { value = readFileSync(resolve(secretDirectory, reference), 'utf8').replace(/\r?\n$/u, ''); } catch { fail(`secret reference for ${name} could not be read`); }
  ...
  for (const [name, value] of Object.entries(inherited)) {
    ...
    if (APPLICATION_NAMES.test(name) && !Object.hasOwn(managed, name)) fail(`runtime value for ${name} is not declared in the release`);
  }
...
function composeRelease(release, releaseFile, secretDirectory) {
  ...
  for (const side of SIDES) {
    ...
    services[side] = {
      ...
      volumes: [
        ...
        ...Object.values(service.secrets).map(reference => ({ type: 'bind', source: resolve(secretDirectory, reference), target: `/run/draken-secrets/${reference}`, read_only: true })),
  ...
  return { name: `draken-${release.dragon.toLowerCase()}-${release.environment}`, services, volumes: { data: { external: true, name: release.dataVolume } } };
}
